...
(function () {
  ...
  /* Show the true final price: the $2 only enters the cart at checkout
     click, so while Full is selected the displayed subtotal is bumped by
     $2 in place (and restored on Reduced). The theme re-renders these
     spans on every cart change; the observer re-applies. dataset guards
     make the write idempotent (no observer loops, no double-adds). */
  function adjust() {
    var want = pref() === 'full' && !!document.querySelector('[data-lf-pack]');
