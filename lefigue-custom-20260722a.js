...
/* Packaging choice (2026-07-21; v3 model 2026-07-22). Per-ORDER, BOTH
   FREE: Full packaging (signature box & pouch, default) vs Reduced
   (pouch only). The choice rides as cart attribute "Packaging" — the
   chooser radios are named attributes[Packaging] inside the cart forms
   (native checkout POST carries it), and this module ALSO persists it
   via /cart/update.js on every toggle + on load, covering dynamic
   checkout buttons that bypass the form. Preference remembered in
   localStorage "lf:packaging" (absent = full). scrub() removes legacy
   v1 "$2 Full Packaging" lines from old carts. Chooser markup in
...
(function () {
  ...
  function label(p) {
    return p === 'reduced' ? 'Reduced packaging' : 'Full packaging';
  }
  function paint() {
    var want = label(pref());
  ...
  function save() {
    return fetch('/cart/update.js', {
      ...
      body: JSON.stringify({ attributes: { Packaging: label(pref()) } })
    }).catch(function () {});
  }
  ...
  document.addEventListener('change', function (e) {
    ...
    if (!r) return;
    var v = r.value === 'Reduced packaging' ? 'reduced' : 'full';
    try { localStorage.setItem(KEY, v); } catch (err) {}
    paint();
    save();
  });

  // On load: drop legacy v1 packaging lines, and make sure the attribute
  // matches the remembered choice (first visit stamps the Full default).
  function sync() {
    fetch('/cart.js', { credentials: 'same-origin' })
      .then(function (r) { return r.json(); })
      .then(function (c) {
        var hasLegacy = (c.items || []).some(function (i) { return i.variant_id === LEGACY_VARIANT; });
        if (hasLegacy) {
          var updates = {};
        ...
        var attr = (c.attributes || {}).Packaging;
        if (hasItems && attr !== label(pref())) save();
      })
      .catch(function () {});
  }
